...
if [ -f "$GD/python/extract.py" ]; then
  if command -v python3 >/dev/null; then
    python3 "$GD/python/extract.py" "$AL/python.json" "$RAW/python.raw.json" && ok=1 || echo "WARN: Python extractor failed" >&2
  else
...
if [ -f "$GD/ts/extract.mjs" ]; then
  node "$GD/ts/extract.mjs" "$AL/javascript.json" "$RAW/javascript.raw.json" && ok=1 || echo "WARN: JavaScript extractor failed" >&2
fi

if [ -f "$GD/java/Extract.java" ]; then
  if command -v java >/dev/null; then
    node -e "console.log(require('./$AL/java.json').map(e => e.class + ' ' + e.recv).join('\n'))" > "$RAW/java.allow" &&
      java "$GD/java/Extract.java" "$RAW/java.allow" "$RAW/java.raw.json" && ok=1 || echo "WARN: Java extractor failed" >&2
  else
...
if [ -f "$GD/go/extract.go" ]; then
  if command -v go >/dev/null; then
    go run "$GD/go/extract.go" "$AL/go.json" "$RAW/go.raw.json" && ok=1 || echo "WARN: Go extractor failed" >&2
  else
